...
const MenuView: React.FC<MenuViewProps> = ({ companyId }) => {
  ...
  const { addToCart, setTableId, setTableNo, tableNo, tableId } = useCart();

  useEffect(() => {
    // Sync table from URL to Cart Context
    if (urlTableId && urlTableId !== tableId) {
       setTableId(urlTableId);
    }
    if (urlTableNo && urlTableNo !== tableNo) {
       setTableNo(urlTableNo);
    }
  }, [urlTableId, urlTableNo, tableId, tableNo, setTableId, setTableNo]);
  ...
  const filteredItems = products.filter(item => {
    const itemCatId = (item.category_id as any)?._id || item.category_id;
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans pb-24">
      ...
      <div className="container mx-auto px-4 -mt-8 relative z-10">
        {/* Table Notification */}
        {tableNo && (
          <div className="mb-6 p-4 bg-white/80 backdrop-blur-md border border-white rounded-2xl flex items-center gap-3 text-slate-900 shadow-xl shadow-slate-200/50 border">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {filteredItems.map(item => (
              <div
                ...
                className="bg-white rounded-[2.5rem] border border-slate-100 overflow-hidden group hover:shadow-2xl hover:shadow-slate-200/50 transition-all flex flex-col border"
              >
