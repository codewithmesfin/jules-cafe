...
export default function UnitsPage() {
  ...
  return (
    <RoleGuard allowedRoles={['admin', 'manager']}>
      <div className="space-y-8 animate-in fade-in duration-500">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          ...
          <div className="flex gap-3">
             ...
             <Button onClick={() => { setEditingConv(null); setConvForm({from_unit:'', to_unit:'', factor:1}); setIsConvModalOpen(true); }} className="rounded-2xl h-12 px-6 bg-blue-600 hover:bg-blue-700 font-bold shadow-lg shadow-blue-100"><RefreshCcw size={18} className="mr-2" /> Add Conversion</Button>
          </div>
        </div>
      ...
      <Modal isOpen={isConvModalOpen} onClose={() => setIsConvModalOpen(false)} title={editingConv ? "Update Conversion" : "New Unit Conversion"} className="max-w-md" footer={<div className="flex gap-3 w-full"><Button variant="outline" onClick={() => setIsConvModalOpen(false)} className="flex-1 rounded-xl h-12 font-bold">Cancel</Button><Button onClick={handleSaveConv} className="flex-1 rounded-xl h-12 bg-blue-600 hover:bg-blue-700 font-black shadow-lg shadow-blue-100">Save Conversion</Button></div>}>
