...
export const getAllInventory = async (req: Request, res: Response) => {
  try {
    const doc = await InventoryItem.find({ is_active: true })
...
export const getOneInventory = async (req: Request, res: Response) => {
  try {
...
export const createInventory = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (item.item_type !== 'inventory') {
      return res.status(400).json({ error: 'Selected item must be of type "inventory"' });
    }
    ...
    if (existing) {
      return res.status(400).json({ error: 'Inventory item already exists for this branch. Use update instead.' });
    }
    ...
    if (!populatedDoc) {
      return res.status(500).json({ error: 'Failed to fetch created inventory item' });
    }
